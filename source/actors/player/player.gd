...
#Variables needed to maintain the player on the same
#screen position when running
...
func _fixed_process(delta):
	#just to debug the current state of
	#the player, I need to remeber to
	#erase it
...
func applyCustomForces(delta):
	...
	#custom function to apply this single
	#custom force, but who knows, maybe
...
func jump(delta):
	...
	if playerCurrentState != playerStates.SLIDING:
		if Input.is_action_just_pressed("ui_accept") and canJump:
			...
			currentPos = get_pos()
			canJump = false
		#This verification takes the position
		#where the jump started, so the player
		#alwyas have a limited max height, but
		...
		if Input.is_action_pressed("ui_accept") and lastPos.distance_to(currentPos) < jumpHeight and canJump:
			...
			#just for future verifications using
			#the state machine
...
func slide():
	...
	if is_colliding():
		if playerCurrentState != playerStates.JUMPING and collider != null and collider.is_in_group("platform"):
			#The player has full control on how much
			...
			if Input.is_action_just_pressed("ui_down"):
				...
				#timer so that it doesn't reset it again
				playerCurrentState = playerStates.RUNNING
